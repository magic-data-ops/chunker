#!/usr/bin/env python3
"""Generate Q&A evaluation chains using OpenCode as the agent harness.

OpenCode's built-in grep and read tools navigate the text corpus autonomously —
no custom FAISS hop loop needed.  For each category we create one persistent
OpenCode session and send batched prompts asking the agent to grep/read relevant
passages and return structured JSON.
...
Usage:
    ...
    python 2_generate_qa_chains.py --rank 0 --world_size 4  # multi-GPU sharding (categories level)

The OpenCode server must be running.  Launch it first with:
    opencode serve --port 4096
or use launch_qa_gen.py which starts it automatically.
"""
...
CATEGORIES_CFG = Path(__file__).parent / "qa_config" / "categories.yaml"
...
def _pairs_to_chains(pairs: List[dict], category: dict, seed_file: str) -> List[dict]:
    ...
    for pair in pairs:
        ...
        # Build a synthetic hop_path from the evidence snippets
        hop_path = [
            {
                "hop_index": i,
...
async def generate_for_category(
    ...
    """Generate n_chains Q&A pairs for one category using a persistent OpenCode session."""
    ...
    # Use first file as default seed hint for schema
    seed_file = txt_files[0].name

    session_id = await client.create_session(title=f"qa_{category['name']}")
    chains: List[dict] = []
    ...
    try:
        ...
        while remaining > 0:
            ...
            reply = await client.send_prompt(session_id, AGENT_NAME, prompt)
            ...
            if pairs:
                ...
                reminder = (
                    "Please output ONLY a valid JSON array as specified. "
                    f"No explanation. Generate {batch} Q&A pairs now."
                )
                reply2 = await client.send_prompt(session_id, AGENT_NAME, reminder)
                pairs2 = _extract_json_array(reply2)
                if pairs2:
                    new_chains = _pairs_to_chains(pairs2, category, seed_file)
                    chains.extend(new_chains)
                    remaining -= len(new_chains)
                else:
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Q&A chains via OpenCode agent")
    parser.add_argument("--corpus_text_dir", default="corpus_text",
                        help="Directory of .txt files that OpenCode can grep/read")
    ...
    # Wait for server
    print("Waiting for OpenCode server...")
    await client.wait_until_ready()
    print(f"Server ready. Generating {per_category} chains × {len(rank_categories)} categories.")
    ...
    for coro in atqdm(asyncio.as_completed(tasks), total=len(tasks), desc="Categories"):
